 ...
 public class Module {
   ...
   public int defaultGVol = 64; public int defaultSpeed = 6; public int defaultTempo = 125; public int c2Rate = 8287; public int gain = 64; public boolean linearPeriods = false;
   ...
   private void loadMod(Data moduleData) throws IOException {
     ...
     for (int seqIdx = 0; seqIdx < 128; seqIdx++) {
       int i = moduleData.uByte(952 + seqIdx) & 0x7F;
       this.sequence[seqIdx] = i;
       if (i >= this.numPatterns) this.numPatterns = i + 1; 
     } 
     ...
     for (int patIdx = 0; patIdx < this.numPatterns; patIdx++) {
       ...
       for (int patDataIdx = 0; patDataIdx < pattern.data.length; patDataIdx += 5) {
         ...
         if (param == 0 && (effect < 3 || effect == 10)) effect = 0; 
   ...
   private void loadS3M(Data moduleData) throws IOException {
     ...
     this.fastVolSlides = ((flags & 0x40) == 64 || version == 4864);
     ...
     for (int instIdx = 1; instIdx <= this.numInstruments; instIdx++) {
       ...
       if (moduleData.uByte(instOffset) == 1 && 
         moduleData.uleShort(instOffset + 76) == 17235) {
         ...
         boolean loopOn = ((moduleData.uByte(instOffset + 31) & 0x1) == 1);
         if (loopStart + loopLength > sampleLength)
           loopLength = sampleLength - loopStart; 
         ...
         boolean stereo = ((moduleData.uByte(instOffset + 31) & 0x2) == 2);
         boolean sixteenBit = ((moduleData.uByte(instOffset + 31) & 0x4) == 4);
     ...
     for (int patIdx = 0; patIdx < this.numPatterns; patIdx++) {
       ...
       while (rowIdx < 64) {
         ...
         if ((token & 0x20) == 32) {
           ...
           if (noteKey == 255) noteKey = 0; 
         } 
         int noteVol = 0;
         if ((token & 0x40) == 64) {
           noteVol = (moduleData.uByte(inOffset++) & 0x7F) + 16;
           if (noteVol > 80) noteVol = 0; 
         } 
         int noteEffect = 0;
         int noteParam = 0;
         if ((token & 0x80) == 128) {
           ...
           if (noteEffect > 0) noteEffect += 128; 
         } 
         int j = channelMap[token & 0x1F];
         if (j >= 0) {
           int noteOffset = (rowIdx * this.numChannels + j) * 5;
     ...
     for (int i = 0; i < 32; i++) {
       if (channelMap[i] >= 0) {
         ...
         if (stereoMode) {
           panning = 12;
           if (moduleData.uByte(64 + i) < 8) panning = 3; 
         } 
         if (defaultPan) {
           int panFlags = moduleData.uByte(moduleDataIdx + i);
           if ((panFlags & 0x20) == 32) panning = panFlags & 0xF; 
         } 
         this.defaultPanning[channelMap[i]] = panning * 17;
       } 
     } 
   }
   private void loadXM(Data moduleData) throws IOException {
     ...
     this.gain = 64;
     this.defaultPanning = new int[this.numChannels];
     for (int idx = 0; idx < this.numChannels; ) { this.defaultPanning[idx] = 128; idx++; }
      this.sequence = new int[this.sequenceLength];
     for (int seqIdx = 0; seqIdx < this.sequenceLength; seqIdx++) {
     ...
     for (int patIdx = 0; patIdx < this.numPatterns; patIdx++) {
       ...
       if (patternDataLength > 0) {
         ...
         for (int note = 0; note < numNotes; note++) {
           ...
           if ((flags & 0x80) == 0) { flags = 31; } else { dataOffset++; }
            byte key = ((flags & 0x1) > 0) ? moduleData.sByte(dataOffset++) : 0;
           pattern.data[patternDataOffset++] = key;
           byte ins = ((flags & 0x2) > 0) ? moduleData.sByte(dataOffset++) : 0;
           pattern.data[patternDataOffset++] = ins;
           byte vol = ((flags & 0x4) > 0) ? moduleData.sByte(dataOffset++) : 0;
           pattern.data[patternDataOffset++] = vol;
           byte fxc = ((flags & 0x8) > 0) ? moduleData.sByte(dataOffset++) : 0;
     ...
     for (int insIdx = 1; insIdx <= this.numInstruments; insIdx++) {
       ...
       if (numSamples > 0) {
         ...
         pointTick = 0;
         for (int i = 0; i < 12; i++) {
           int pointOffset = dataOffset + 177 + i * 4;
           pointTick = (deltaEnv ? pointTick : 0) + moduleData.uleShort(pointOffset);
           panEnv.pointsTick[i] = pointTick;
           panEnv.pointsAmpl[i] = moduleData.uleShort(pointOffset + 2);
         } 
