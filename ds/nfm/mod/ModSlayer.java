   ...
   extends ModuleSlayer
 {
   ...
   final int get_track(ModTrackInfo modtrackinfo, byte[] is, int i) {
     ...
     if (modtrackinfo.noterestart < -1) modtrackinfo.noterestart = -1; 
     if (i_3_ != 0) {
     ...
     if (i_4_ != 0 || i_5_ != 0)
     { int i_6_; int i_7_; switch (i_4_)
       { default:
         
         case 0:
           i_6_ = 12;
           for (; i_6_ < 48 && modtrackinfo.period < period_set[i_6_]; i_6_++);
         ...
         case 14:
           ...
           { 
             ...
             default:
               return i;case 1: modtrackinfo.period += i_5_; if (modtrackinfo.period > modtrackinfo.period_high_limit) modtrackinfo.period = modtrackinfo.period_high_limit;  modtrackinfo.pitch = modtrackinfo.finetune_rate / modtrackinfo.period;case 2: modtrackinfo.period -= i_5_; if (modtrackinfo.period < modtrackinfo.period_low_limit) modtrackinfo.period = modtrackinfo.period_low_limit;  modtrackinfo.pitch = modtrackinfo.finetune_rate / modtrackinfo.period;case 6: if (this.loops == 0 && !this.onLoop) if (i_5_ <= 0) { this.loopA = this.row; } else { this.loopB = this.row; this.loops = i_5_; this.onLoop = true; }  case 9: modtrackinfo.noterestart = i_5_;case 10: modtrackinfo.volume += i_5_; if (modtrackinfo.volume > 64)
                 modtrackinfo.volume = 64;  if (modtrackinfo.volume < 0)
                 modtrackinfo.volume = 0; case 11: modtrackinfo.volume -= i_5_; if (modtrackinfo.volume > 64)
                 modtrackinfo.volume = 64;  if (modtrackinfo.volume < 0)
                 modtrackinfo.volume = 0; case 12: break; }  modtrackinfo.notelimit = i_5_;case 15: break; }  if (i_5_ != 0) { i_5_ &= 0xFF; if (i_5_ <= 32) { this.tempo = i_5_; this.tempo_wait = i_5_; }  this.bpm = i_5_; this.bpm_samples = this.samplingrate / (103 * i_5_ >> 8) * this.oversample; }  }  } final void make_vol_table8() { this.vol_table = new byte[16640];
         ...
         if (modtrackinfo.pitch < 4096) {
           while (samplepos < endtr && buffpos < buffend) {
             if (modtrackinfo.notelimit == -1 || this.tempo - this.tempo_wait < modtrackinfo.notelimit) {
               buffer[buffpos++] = buffer[buffpos++] + ((samples[samplepos] * (4096 - error) + samples[samplepos + 1] * error) * volume >> 12);
             } else {
               modtrackinfo.volume = 0;
           ...
           while (samplepos < endtr && buffpos < buffend) {
             if (modtrackinfo.notelimit == -1 || this.tempo - this.tempo_wait < modtrackinfo.notelimit) {
               buffer[buffpos++] = buffer[buffpos++] + samples[samplepos] * volume;
             } else {
               modtrackinfo.volume = 0;
             }  samplepos += hipitch + ((error += lopitch) >> 12);
             error &= 0xFFF;
   ...
   public byte[] turnbytesNorm(boolean calvol) throws IOException {
     ...
     while (!this.mod_done) {
       if (--this.tempo_wait > 0) {
         ...
         System.arraycopy(emptybuf, 0, buf, 0, this.bpm_samples); int i;
         for (i = 0; i < this.numtracks; i++) {
           mixtrack_16_mono(this.tracks[i], buf, 0, this.bpm_samples);
         }
   ...
   final void startplaying(boolean flag) {
     ...
     if (this.mod.s3m) {
       for (int j = 0; j < this.mod.insts.length; j++) {
         ...
         modinstrument.finetune_rate = (int)(428L * modinstrument.finetune_value << 8L) / this.mixspeed;
   ...
   final void updatetracks() {
     ...
     if (this.row >= 64) {
       ...
       if (this.loops > 0 && 
         this.loops > 0 && this.row == this.loopB) {
